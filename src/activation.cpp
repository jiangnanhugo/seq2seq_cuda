...
namespace seq2seq {
	...
	void Activation_function::forward(Blob* input, Blob* output) {
		...
		cudnnErrCheck(cudnnActivationForward(GlobalAssets::instance()->cudnnHandle(), _activ_desc,
	...
	void Activation_function::backward(Blob* input, Blob* output) {
		cudnnErrCheck(cudnnActivationBackward(GlobalAssets::instance()->cudnnHandle(), _activ_desc,
			cudnn::dataType<float>::one, _output_desc, output->device_w, _output_desc,
