...
void RNN_layer::init(int batch_size, int hidden_size, int input_size,
  ...
  cudnnErrCheck(cudnnSetRNNDescriptor(
      GlobalAssets::instance()->cudnnHandle(), _rnn_desc, _hidden_size,
      _num_layers,
  ...
  cudnnErrCheck(cudnnGetRNNParamsSize(GlobalAssets::instance()->cudnnHandle(),
                                      _rnn_desc, _x_desc[0], &_weights_size,
                                      CUDNN_DATA_FLOAT));
...
void RNN_layer::initialize_params() {
  ...
  for (int layer = 0; layer < total_layers; ++layer) {
    ...
    for (int lin_layer_id = 0; lin_layer_id < num_linear_layers; ++lin_layer_id) {
      ...
      cudnnErrCheck(cudnnGetRNNLinLayerMatrixParams(
          ...
          _w_desc, _w->get(), lin_layer_id, linLayerMatDesc,
          (void **)&linLayerMat));
      ...
      // fprintf(stderr,"filterDimA: %d %d %d\n", filterDimA[0], filterDimA[1],
      // filterDimA[2]);
      initGPUData(linLayerBias, filterDimA[0] * filterDimA[1] * filterDimA[2],
                  0.1);
  ...
  for (size_t i = 0; i < _matrix_blobs.size(); ++i) {
    for (size_t j = 0; j < _matrix_blobs[i].size(); ++j) {
      ...
      matrix.host_w = (float *)malloc(shape * sizeof(float));
      assert(matrix.host_w != NULL);
      matrix.host_g = (float *)malloc(shape * sizeof(float));
      assert(matrix.host_g != NULL);

      cudaErrCheck(cudaMalloc((void**)&matrix.device_g, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(matrix.device_g, 0.0, shape * sizeof(float)));

      cudaErrCheck(cudaMalloc((void**)&matrix.device_m, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(matrix.device_m, 0.0, shape * sizeof(float)));

      cudaErrCheck(cudaMalloc((void**)&matrix.device_v, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(matrix.device_v, 0.0, shape * sizeof(float)));
  ...
  for (size_t i = 0; i < _bias_blobs.size(); ++i) {
    for (size_t j = 0; j < _bias_blobs[i].size(); ++j) {
      ...
      cudaErrCheck(cudaMalloc((void**)&bias.device_g, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(bias.device_g, 0.0, shape * sizeof(float)));

      cudaErrCheck(cudaMalloc((void**)&bias.device_m, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(bias.device_m, 0.0, shape * sizeof(float)));

      cudaErrCheck(cudaMalloc((void**)&bias.device_v, shape * sizeof(float)));
      cudaErrCheck(cudaMemset(bias.device_v, 0.0, shape * sizeof(float)));
  ...
  // TODO: be aware of this usage, especially when adding codes to free these
  // memory
