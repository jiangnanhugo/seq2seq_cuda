...
namespace seq2seq{
    void RNN_layer::init(int batch_size, int hidden_size, int input_size,
		...
		cudnnErrCheck(cudnnSetDropoutDescriptor(_dropout_desc,
			GlobalAssets::instance()->cudnnHandle(), _dropout, _states->get(), state_size, seed));

		// -------------------------
		// Set up the RNN descriptor
		// -------------------------
		cudnnErrCheck(cudnnCreateRNNDescriptor(&_rnn_desc));
		cudnnErrCheck(cudnnSetRNNDescriptor(GlobalAssets::instance()->cudnnHandle(),
			_rnn_desc,
			_hidden_size, _num_layers,
	...
	void RNN_layer::initialize_params() {
		...
		for (int layer = 0; layer < total_layers; ++layer) {
			...
			for (int lin_layer_id = 0; lin_layer_id < num_linear_layers; ++lin_layer_id) {
				...
				cudnnErrCheck(cudnnGetRNNLinLayerBiasParams(GlobalAssets::instance()->cudnnHandle(),
					_rnn_desc,
					layer,
					_x_desc[0],
					_w_desc,
