...
namespace seq2seq {
    enum class sort_type {SMALL, LARGE, COMBO};
    ...
    void cross_entropy_loss_bp(const float* input, const float* labels, float* output, int batch,int num_labels,float loss_factor,int pad_id);

    void focal_loss_ff(const float* input,const float* labels, float* output,int batch,int num_labels, float gamma, int pad_id);
    ...
    void compute_context(const float* attention_weights, const float* encoder_hidden, float* context, int seq_len, int batch_size,
            int hidden_size);

    void bp_compute_context( const float* context_diff, const float* attention_weights, const float* encoder_hidden,
            float* attention_weights_diff, float* encoder_hidden_diff, int seq_len, int batch_size, int hidden_size);
    ...
    void maxout_bp( float* pre_maxout_diff, const float* maxout_diff, const float* maxout_ele_idx, int total_output_size);
